AddCSLuaFile("autorun/acf_missile/guidance/guidance_dumb.lua")
include("autorun/acf_missile/guidance/guidance_dumb.lua")

AddCSLuaFile("autorun/acf_missile/guidance/guidance_wire.lua")
include("autorun/acf_missile/guidance/guidance_wire.lua")

AddCSLuaFile("autorun/acf_missile/guidance/guidance_laser.lua")
include("autorun/acf_missile/guidance/guidance_laser.lua")

AddCSLuaFile("autorun/acf_missile/guidance/guidance_radar.lua")
include("autorun/acf_missile/guidance/guidance_radar.lua")

AddCSLuaFile("autorun/acf_missile/guidance/guidance_antimissile.lua")
include("autorun/acf_missile/guidance/guidance_antimissile.lua")


AddCSLuaFile("autorun/acf_missile/guidance/guidance_infrared.lua")
include("autorun/acf_missile/guidance/guidance_infrared.lua")
